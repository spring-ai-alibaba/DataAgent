...
public class DynamicFilterService {
        ...
        switch (vectorType) {
            ...
            case DocumentMetadataConstant.AGENT_KNOWLEDGE:
                ...
                if (validIds.isEmpty()) {
                    ...
                    // 加入 ID 过滤
                    conditions.add(b.in(DocumentMetadataConstant.DB_AGENT_KNOWLEDGE_ID, validIds.toArray(new Integer[0])).build());
                }
                break;
            ...
            case DocumentMetadataConstant.BUSINESS_TERM:
                ...
                if (recalledBusinessKnowledgeIds.isEmpty()) {
                    ...
                    // 添加 ID 过滤
                    conditions
                            .add(b.in(DocumentMetadataConstant.DB_BUSINESS_TERM_ID, recalledBusinessKnowledgeIds.toArray(new Long[0]))
                                    .build());
                }
                break;
    ...
    public static Filter.Expression buildFilterExpressionForSearchTables(String agentId, List<String> tableNames) {
        ...
        if (tableNames != null && !tableNames.isEmpty()) {
            conditions.add(b.in(DocumentMetadataConstant.NAME, tableNames.toArray(new String[0])).build());
        }
        else {
