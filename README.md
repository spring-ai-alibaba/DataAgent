...
如果你想先确认主链路已经跑通，可以先用下面这条最小流程做一次验证：

1. 在管理端先完成一个可用模型配置，并确保后端可以正常调用该模型。
2. 在“数据源管理”里添加一个 MySQL 或 PostgreSQL 数据源，并用测试表完成一次连接校验。
3. 在智能体中挂载这个数据源后，输入一条简单问题，例如“统计商品表中的商品总数”。
4. 先观察是否产生 SQL 执行结果，再继续查看是否生成图表或最终分析报告。

推荐优先使用仓库自带的 `product_schema.sql` / `product_data.sql` 示例数据完成第一次验证。这样可以先确认 SQL 生成、执行和结果回传链路都工作正常，再继续调试 Python 深度分析、RAG 或 MCP 能力。
