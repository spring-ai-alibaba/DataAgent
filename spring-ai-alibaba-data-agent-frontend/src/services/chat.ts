...
class ChatService {
    ...
    async createSession(agentId: number, title?: string, userId?: number): Promise<ChatSession> {
        ...
        const response = await axios.post<ChatSession>(`${API_BASE_URL}/agent/${agentId}/sessions`, request);
        return response.data;
    }
    ...
    async getSessionMessages(sessionId: string): Promise<ChatMessage[]> {
        const response = await axios.get<ChatMessage[]>(`${API_BASE_URL}/sessions/${sessionId}/messages`);
        return response.data;
    }
    ...
    async saveMessage(sessionId: string, message: ChatMessage): Promise<ChatMessage> {
        try {
            ...
            const response = await axios.post<ChatMessage>(`${API_BASE_URL}/sessions/${sessionId}/messages`, messageData);
            return response.data;
        } catch (error) {
    ...
    async pinSession(sessionId: string, isPinned: boolean): Promise<ApiResponse> {
        try {
            const response = await axios.put<ApiResponse>(`${API_BASE_URL}/sessions/${sessionId}/pin`, null, {
                params: { isPinned }
            });
            return response.data;
        } catch (error) {
    ...
    async renameSession(sessionId: string, title: string): Promise<ApiResponse> {
        try {
            ...
            const response = await axios.put<ApiResponse>(`${API_BASE_URL}/sessions/${sessionId}/rename`, null, {
