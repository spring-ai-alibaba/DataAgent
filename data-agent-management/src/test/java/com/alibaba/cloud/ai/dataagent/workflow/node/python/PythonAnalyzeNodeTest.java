...
class PythonAnalyzeNodeTest {
	...
	void apply_llmFailure_throwsException() {
		...
		when(llmService.callSystem(anyString(), org.mockito.ArgumentMatchers.any())).thenThrow(new RuntimeException("LLM service unavailable"));
	...
	void apply_timeoutInLlmAnalysis_returnsResultWithGenerator() throws Exception {
		...
		when(llmService.callSystem(anyString(), org.mockito.ArgumentMatchers.any())).thenReturn(Flux.error(new RuntimeException("LLM analysis timeout")));
