...
class ResponsesApiChatModelTest {
	...
	@Test
	void call_openAiRuntimeOptionsMapThinkingAndReasoningEffort() {
		when(responsesApi.call(any())).thenReturn(completedResponse("ok"));

		OpenAiChatOptions runtimeOptions = OpenAiChatOptions.builder()
			.reasoningEffort("high")
			.extraBody(java.util.Map.of("thinking", java.util.Map.of("type", "enabled")))
			.build();
	...
	@Test
	void call_disabledThinkingClearsDefaultReasoningEffort() {
		when(responsesApi.call(any())).thenReturn(completedResponse("ok"));
		OpenAiChatOptions defaults = OpenAiChatOptions.builder()
			...
			.extraBody(java.util.Map.of("thinking", java.util.Map.of("type", "enabled")))
			.build();
		chatModel = new ResponsesApiChatModel(responsesApi, defaults);
		OpenAiChatOptions runtimeOptions = OpenAiChatOptions.builder()
	...
	@Test
	void stream_systemOnlyPrompt_addsRequiredInput() {
		when(responsesApi.stream(any())).thenReturn(Flux.empty());

		chatModel.stream(new Prompt(List.of(new SystemMessage("generate sql")))).collectList().block();

		ArgumentCaptor<ResponsesRequest> captor = ArgumentCaptor.forClass(ResponsesRequest.class);
		verify(responsesApi).stream(captor.capture());
		ResponsesRequest request = captor.getValue();
		assertEquals("generate sql", request.instructions());
		assertNotNull(request.input());
		assertEquals(1, request.input().size());
		assertEquals("user", request.input().get(0).role());
		assertFalse(request.input().get(0).content().get(0).text().isBlank());
	}
