...
public class ModelConfigConverter {
	...
	public static ModelConfigDTO toDTO(ModelConfig entity) {
		...
		return ModelConfigDTO.builder()
			...
			.thinkingEnabled(Boolean.TRUE.equals(entity.getThinkingEnabled()))
			.reasoningEffort(StringUtils.hasText(entity.getReasoningEffort()) ? entity.getReasoningEffort() : "high")
			.build();
	}
	...
	public static ModelConfig toEntity(ModelConfigDTO dto) {
		...
		entity.setThinkingEnabled(Boolean.TRUE.equals(dto.getThinkingEnabled()));
		entity.setReasoningEffort(StringUtils.hasText(dto.getReasoningEffort()) ? dto.getReasoningEffort() : "high");
		// 默认值处理
