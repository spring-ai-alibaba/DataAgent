...
/**
 * Agent Schema Initialization Controller Handles agent's database Schema initialization to vector
 * storage
 */
@Slf4j
@RestController
...
public class AgentDatasourceController {
  ...
  /**
   * Initialize agent's database Schema to vector storage Corresponds to the "Initialize Information
   * Source" function on the frontend
   */
  @PostMapping("/init")
  public ApiResponse initSchema(@PathVariable Long agentId) {
    ...
    try {
      ...
      List<String> tables =
          Optional.ofNullable(agentDatasource.getSelectTables()).orElse(List.of());
      ...
      Boolean result =
          agentDatasourceService.initializeSchemaForAgentWithDatasource(
              agentId, datasourceId, tables);

      if (result) {
        log.info(
            "Successfully initialized schema for agent: {}, tables: {}", agentId, tables.size());
  ...
  @GetMapping
  public ApiResponse getAgentDatasource(@PathVariable Long agentId) {
    try {
  ...
  @GetMapping("/active")
  public ApiResponse getActiveAgentDatasource(@PathVariable Long agentId) {
    try {
  ...
  public ApiResponse addDatasourceToAgent(
      @PathVariable Long agentId, @PathVariable Integer datasourceId) {
    try {
      if (datasourceId == null) {
        throw new InvalidInputException("数据源ID不能为空");
      }
      ...
      AgentDatasource agentDatasource =
          agentDatasourceService.addDatasourceToAgent(agentId, datasourceId);
  ...
  // 更新选择的数据表
  @PostMapping("/tables")
  public ApiResponse updateDatasourceTables(
      @PathVariable Long agentId, @RequestBody @Validated UpdateDatasourceTablesDTO dto) {
    try {
      dto.setTables(Optional.ofNullable(dto.getTables()).orElse(List.of()));
      agentDatasourceService.updateDatasourceTables(
          agentId, dto.getDatasourceId(), dto.getTables());
  ...
  public ApiResponse removeDatasourceFromAgent(
      @PathVariable Long agentId, @PathVariable Integer datasourceId) {
    try {
  ...
  /** 启用/禁用智能体的数据源 */
  @PutMapping("/toggle")
  public ApiResponse toggleDatasourceForAgent(
      @PathVariable Long agentId, @RequestBody ToggleDatasourceDTO dto) {
    try {
      ...
      AgentDatasource agentDatasource =
          agentDatasourceService.toggleDatasourceForAgent(agentId, dto.getDatasourceId(), isActive);
