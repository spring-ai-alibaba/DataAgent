...
public class AgentStartupInitialization implements ApplicationRunner, DisposableBean {
  ...
  public void run(ApplicationArguments args) {
    ...
    try {
      ...
      CompletableFuture.runAsync(this::initializePublishedAgents, executorService)
          .exceptionally(
              throwable -> {
  ...
  private void initializePublishedAgents() {
    try {
      ...
      for (Agent agent : publishedAgents) {
        try {
          ...
          if (initialized) {
            ...
            failureCount++;
            log.warn(
                "Failed to initialize agent: {} (ID: {}) - no active datasource or tables",
                agent.getName(),
                agent.getId());
          }
        } catch (Exception e) {
          failureCount++;
          log.error(
              "Error initializing agent: {} (ID: {}, reason: {})",
              agent.getName(),
              agent.getId(),
              e.getMessage());
        }
      ...
      log.info(
          "Agent initialization completed. Success: {}, Failed: {}, Total: {}",
          successCount,
          failureCount,
  ...
  private boolean initializeAgentDataSource(Agent agent) {
    try {
      ...
      log.info(
          "Initializing agent {} with datasource {} and {} tables",
          agentId,
          datasourceId,
          tables.size());
      ...
      if (result) {
        log.info(
            "Successfully initialized datasource for agent {} with {} tables",
            agentId,
            tables.size());
        return true;
      } else {
      ...
      log.error(
          "Error initializing datasource for agent {}, reason: {}", agent.getId(), e.getMessage());
      return false;
    }
  }
  ...
  private boolean isAlreadyInitialized(Long agentId) {
    try {
      ...
      log.error(
          "Failed to check initialization status for agent: {}, assuming not initialized",
          agentId,
          e);
      return false;
    }
  ...
  /**
   * Clean up resources when the application shuts down. Implement the destroy method of the
