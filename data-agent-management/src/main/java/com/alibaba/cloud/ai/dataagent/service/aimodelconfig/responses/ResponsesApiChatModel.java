...
public class ResponsesApiChatModel implements ChatModel {
	...
	private static final String SYSTEM_ONLY_INPUT = "Provide the requested output according to the instructions.";
	...
	private ResponsesRequest buildRequest(Prompt prompt, boolean stream) {
		...
		// Responses API requires input even when the original Chat prompt contains only a
		// SystemMessage. DataAgent's SQL generator uses exactly that shape.
		if (inputItems.isEmpty()) {
			inputItems.add(InputItem.userMessage(SYSTEM_ONLY_INPUT));
		}
		...
		if (runtimeOptions != null) {
			...
			if (runtimeOptions instanceof OpenAiChatOptions openAiOptions) {
				if (StringUtils.hasText(openAiOptions.getReasoningEffort())) {
	...
	private Thinking extractThinking(Map<String, Object> extraBody) {
		if (extraBody == null || !(extraBody.get("thinking") instanceof Map<?, ?> thinking)) {
			return null;
		}
