...
public class ModelConfigDataServiceImpl implements ModelConfigDataService {
	...
	private void clean(ModelConfigDTO dto) {
		...
		if (StringUtils.hasText(dto.getReasoningEffort())) {
			dto.setReasoningEffort(ReasoningEffort.fromCode(dto.getReasoningEffort().trim()).getCode());
		}
		else {
			dto.setReasoningEffort(ReasoningEffort.HIGH.getCode());
		}
		if (dto.getThinkingEnabled() == null) {
			dto.setThinkingEnabled(false);
		}
	}
	...
	private static void mergeDtoToEntity(ModelConfigDTO dto, ModelConfig oldEntity) {
		...
		oldEntity.setThinkingEnabled(dto.getThinkingEnabled());
		oldEntity.setReasoningEffort(dto.getReasoningEffort());
