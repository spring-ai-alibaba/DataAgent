...
@AllArgsConstructor
@Component
public class SequentialThinkingTool implements
    BiFunction<SequentialThinkingTool.Request, ToolContext, String> {
  ...
  public String apply(Request request, ToolContext toolContext) {
    ...
    try {
      ...
      if (request.thoughtNumber() > request.totalThoughts()) {
        request = new Request(
            request.thought(),
            request.nextThoughtNeeded(),
            request.thoughtNumber(),
            request.thoughtNumber(), // Update totalThoughts
            request.isRevision(),
            request.revisesThought(),
            request.branchFromThought(),
            request.branchId(),
      ...
      // Handle branching
      if (request.branchFromThought() != null && request.branchId() != null && !request.branchId()
          .isEmpty()) {
        branches.computeIfAbsent(request.branchId(), k -> new ArrayList<>()).add(request);
      }

      log.info("[Tool][SequentialThinking] Thought: {}", request.thought());

      // Prepare response data
      List<String> branchKeys = new ArrayList<>(branches.keySet());
      boolean incomplete = request.nextThoughtNeeded() || (request.needsMoreThoughts()!=null && request.needsMoreThoughts()) ||
      ...
      log.info(
          "[Tool][SequentialThinking] Execute completed - Thought {}/{}",
          request.thoughtNumber(),
  ...
  public record Request(
      @JsonProperty(value = "thought", required = true)
      @JsonPropertyDescription("Your current thinking step. Write in natural, user-friendly language. NEVER mention tool names (like \"list_table_schema\", \"sql_execute\", \"web_search\", etc.). Instead, describe actions in plain language (e.g., \"I'll search for key terms\" instead of \"I'll use list_table_schema\"). Focus on WHAT you're trying to find and WHY, not HOW (which tools you'll use).")
      String thought,

      @JsonProperty(value = "next_thought_needed", required = true)
      @JsonPropertyDescription("Whether another thought step is needed")
      boolean nextThoughtNeeded,

      @JsonProperty(value = "thought_number", required = true)
      @JsonPropertyDescription("Current thought number (numeric value, e.g., 1, 2, 3)")
      int thoughtNumber,

      @JsonProperty(value = "total_thoughts", required = true)
      @JsonPropertyDescription("Estimated total thoughts needed (numeric value, e.g., 5, 10)")
      int totalThoughts,

      @JsonProperty(value = "is_revision")
      @JsonPropertyDescription("Whether this revises previous thinking")
      Boolean isRevision,

      @JsonProperty(value = "revises_thought")
      @JsonPropertyDescription("Which thought is being reconsidered")
      Integer revisesThought,

      @JsonProperty(value = "branch_from_thought")
      @JsonPropertyDescription("Branching point thought number")
      Integer branchFromThought,

      @JsonProperty(value = "branch_id")
      @JsonPropertyDescription("Branch identifier")
      String branchId,

      @JsonProperty(value = "needs_more_thoughts")
      @JsonPropertyDescription("If more thoughts are needed")
