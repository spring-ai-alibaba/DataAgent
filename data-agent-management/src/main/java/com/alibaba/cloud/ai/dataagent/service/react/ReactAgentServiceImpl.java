...
public class ReactAgentServiceImpl implements ReactAgentService {
  ...
  public ReactAgentServiceImpl(ExecutorService executor,
      ReactAgentRegistry reactAgentRegistry,
      MultiTurnContextManager multiTurnContextManager,
      ToolsInterceptor toolsInterceptor) throws GraphStateException {
  ...
  @Override
  public void reactStreamProcess(Sinks.Many<ServerSentEvent<GraphNodeResponse>> sink,
    ...
    RunnableConfig config = RunnableConfig.builder().threadId(threadId)
   ...
   * @param threadId       线程ID
   */
  private void subscribeToFlux(StreamContext context, Flux<?> nodeOutputFlux, String agentId,
      String threadId) {
    CompletableFuture.runAsync(() -> {
      ...
      Disposable disposable = nodeOutputFlux.subscribe(
          output -> handleNodeOutput(output, agentId, threadId),
  ...
  private void handleStreamNodeOutput(String threadId, String agentId, StreamingOutput output) {
    ...
    if(RunnableConfig.AGENT_MODEL_NAME.equals(node)) {
      log.debug("handleToolCall: threadId={}, toolName={}, input={}", threadId, node, chunk);
    ...
    if (!isTypeSign) {
      ...
      if (result.isFailure()) {
        log.warn(
            "Failed to emit data to sink for threadId: {}, result: {}. Stopping stream processing.",
            threadId, result);
        // 如果发送失败，停止处理
