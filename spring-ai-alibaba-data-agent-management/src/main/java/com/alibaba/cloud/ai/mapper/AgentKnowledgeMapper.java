...
public interface AgentKnowledgeMapper {
	...
	@Insert({ "INSERT INTO agent_knowledge (agent_id, title, content, type, category, tags, status, ",
	...
	@Update({ "UPDATE agent_knowledge SET",
	...
	@Select({ "SELECT * FROM agent_knowledge WHERE agent_id = #{agentId} AND",
	...
	@Update({ "UPDATE agent_knowledge SET status = #{status}, update_time = #{now} WHERE id = #{id}", })
	...
	@Select({ "SELECT type, COUNT(*) as count FROM agent_knowledge WHERE agent_id = #{agentId} GROUP BY type" })
